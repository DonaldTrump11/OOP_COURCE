...
#==================================
#==================================
# Функция-конструктор для работы
# YAML кода
#==================================
#==================================
...
def create_levels(yaml_code):
    # Загружаем YAML-код
    data = yaml.safe_load(yaml_code)

    # Создаем словарь Levels
    Levels = {'levels': []}

    # Обрабатываем каждый уровень
    for level_data in data.get('levels', []):
        level_map = level_data.get('map')
        level_objects = level_data.get('obj')

        # Создаем объекты Map и Objects для каждого уровня
        map_instance = getattr(globals()[level_map], 'Map')()
        objects_instance = getattr(globals()[level_objects], 'Objects')()

        # Добавляем в словарь Levels
        Levels['levels'].append({'map': map_instance, 'obj': objects_instance})
...
if __name__ == '__main__':
    ...
    # Регистрируем функции-конструкторы
    yaml.add_constructor('!easy_level', create_levels)
    ...
    Levels = create_levels(yaml_code)
    print(Levels)
